...
using PsychicPotato.Extensions;

namespace PsychicPotato.Authentications
{
    public sealed class TokenAuthenticationHandler : AuthenticationHandler<TokenAuthenticationOptions>
    {
        ...
        private readonly PsychicPotatoContext _database;
        ...
        public TokenAuthenticationHandler(IOptionsMonitor<TokenAuthenticationOptions> options, ILoggerFactory logger,
            UrlEncoder encoder, ISystemClock clock, PsychicPotatoContext database) : base(options, logger, encoder, clock)
        {
            _database = database;
